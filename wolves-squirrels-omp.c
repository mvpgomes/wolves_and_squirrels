...
/* pos_equals(struct position* pos1, struct position* pos2) : compares 2 positions */
int pos_equals(struct position* pos1, struct position* pos2) {
	return (pos1->row == pos2->row) && (pos1->column == pos2->column);
}

/* remove_locks(struct position* pos, omp_lock_t *locks) : removes all locks except pos */
...
/* update_periods() : updates breeding and starvation periods across the board */
void update_periods() {
	int i, k;
